...
function isPalindrome(str) {
  const str2 = str.toLowerCase().replaceAll(/[^a-zа-я]+/g, '');
...
function invertCase(str) {
  ...
  return str
    .split('')
    .map((character) =>
      isUpperCase(character) ? character.toLowerCase() : character.toUpperCase()
    )
    .join('');
}
...
function encodeToRot13(str) {
  ...
  const arrayNewCharCode = str.split('').map((character) => {
    ...
    const SYMBOL_ALPHABET = 25;

    const isEnglishLetterLowCase = (symbol) => symbol >= 'a' && symbol <= 'z';
    const isEnglishLetterHighCase = (symbol) => symbol >= 'A' && symbol <= 'Z';

    if (isEnglishLetterLowCase(character)) {
      asciiCode += 13;
      if (!isEnglishLetterLowCase(String.fromCharCode(asciiCode))) {
        asciiCode -= SYMBOL_ALPHABET + 1;
      }
